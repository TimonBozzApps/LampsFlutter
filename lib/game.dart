...
class GameState {
  ...
  GameState(this.sizeX, this.sizeY, this.players, this.playerColors) {
    board = List.generate(
        sizeX,
        (x) => List.generate(sizeY, (y) {
              int maxCharge = 3;
              if (x == 0 || x == sizeX - 1) maxCharge--;
              if (y == 0 || y == sizeY - 1) maxCharge--;
              return Tile("", 0, maxCharge, x, y);
            }));
    exploadingTiles = <Tile>[];
  ...
  GameState.fromPreviousVersionAndExploadingTiles(
      GameState gameState2, this.exploadingTiles)
      : sizeX = gameState2.sizeX,
        sizeY = gameState2.sizeY,
        players = gameState2.players,
  ...
  List<List<Tile>> get playerOwnedTiles {
    var playerOwnedTiles =
        List<List<Tile>>.generate(players.length, (index) => <Tile>[]);
  ...
  List<String> get playersStillInTheGame => playerOwnedTiles
      .where((element) => element.length > 0)
      .map((e) => e[0].owner)
      .toList();

  bool get isTerminated =>
      movesMade > players.length && playersStillInTheGame.length < 2;

  int evaluate(int player) {
    int value = 0;
    for (List<Tile> tiles in board) {
      for (Tile tile in tiles) {
        if (tile.charge == 0) continue;
        int tileValue = tile.charge + 1;
        if (tile.posX == 0 || tile.posX == 4) tileValue *= 2;
        if (tile.posY == 0 || tile.posY == 4) tileValue *= 2;
...
class Game {
  ...
  bool _makeMove(Move move, bool animate) {
    if (!makeMoveCheck(move)) return false;
  ...
  bool makeMoveCheck(Move move) {
    if (!_readyForNextMove) return false;
    ...
    if (_gameState.value.board[move.posX][move.posY].owner !=
            "" && //ok if no one owns it
        _gameState.value.board[move.posX][move.posY].owner != move.player) {
  ...
  void _updateBoard(bool animate) async {
    ...
    List<List<Tile>> newBoard = List.generate(
        _gameState.value.sizeX,
        (x) => List.generate(_gameState.value.sizeY, (y) {
              int maxCharge = 3;
              if (x == 0 || x == _gameState.value.sizeX - 1) maxCharge--;
              if (y == 0 || y == _gameState.value.sizeY - 1) maxCharge--;
              return Tile("", 0, maxCharge, x, y);
            }));
    List<Tile> exploadedTiles = <Tile>[];
    _gameState.value.board.forEach((row) {
      row.forEach((tile) {
        if (tile.charge > tile.maxCharge) {
          ...
          if (newBoard[tile.posX][tile.posY].owner ==
              "") //because explosions obtain tiles
    ...
    if (exploadedTiles.isNotEmpty) {
      if (animate) {
        _gameState.add(GameState.fromPreviousVersionAndExploadingTiles(
            _gameState.value, exploadedTiles));
        await Future.delayed(_animationDuration, () {
          if (!_gameState.isClosed)
            return _gameState.add(GameState.fromPreviousVersionAndBoard(
                _gameState.value, newBoard));
        });
      } else {
        _gameState.value.exploadingTiles = exploadedTiles;
        _gameState.add(
            GameState.fromPreviousVersionAndBoard(_gameState.value, newBoard));
      }
      _updateBoard(animate); //recurse
    } else {
      //find next player
      bool isCurrentPlayerOwningTile = _gameState.value.movesMade <
          _gameState.value.players
              .length; //because in the first round nobody owns a tile
      do {
        int nextPlayerIndex =
            _gameState.value.players.indexOf(_gameState.value.currentPlayer) +
                1;
        if (nextPlayerIndex >= _gameState.value.players.length)
          nextPlayerIndex = 0;
        _gameState.value.currentPlayer =
            _gameState.value.players[nextPlayerIndex];
        //find if current player still has tiles
        if (_gameState.value.playerOwnedTiles[nextPlayerIndex].length > 0)
          isCurrentPlayerOwningTile =
              true; //can't set it to false because of initial condition (^see above^)
  ...
  void printGame() {
    ...
    for (List<Tile> row in board) {
      String rowString = "";
      for (Tile tile in row) rowString += tile.charge.toString();
      print(rowString);
    }
  }
...
class LocalGame extends Game {
  LocalGame(GameState gameState)
      : super(gameState, Duration(milliseconds: 400));
...
class MultiplayerGame extends Game implements LocalGame {
  ...
  MultiplayerGame(this._onlineApi, OnlineGame onlineGame)
      : super(
            GameState(onlineGame.sizeX, onlineGame.sizeY, onlineGame.players,
...
class LocalAiGame extends Game implements LocalGame {
  ...
  LocalAiGame(GameState gameState, this.aiAgents, this.simulateOnline)
    ...
    if (gameState.currentPlayer.startsWith("&&AI&&")) doAiTurn(gameState);
  }

  StreamSubscription aiSub;
  bool makeMove(int posX, int posY) {
    //used for the human input
    if (gameState.value.currentPlayer.startsWith("&&AI&&")) return false;
    return _makeMoveAndStartAi(posX, posY);
  }

  bool _makeMoveAndStartAi(int posX, int posY) {
    final result =
        _makeMove(Move(posX, posY, gameState.value.currentPlayer), true);
    if (!result) return result;
    if (aiSub != null) aiSub.cancel();
    aiSub = gameState.listen((event) async {
      //to wait for animations to be done
      if (_readyForNextMove) {
        aiSub.cancel();
        if (!event.currentPlayer.startsWith("&&AI&&") ||
            event.isTerminated) //not ai's turn
          return;
        //let ai think
        doAiTurn(event);
      }
  ...
  void doAiTurn(GameState state) async {
    if (simulateOnline)
      await Future.delayed(Duration(
          milliseconds: onlineDelays[Random().nextInt(onlineDelays.length)]));
    await Future.delayed(Duration(milliseconds: 500));
    final action = await aiAgents[gameState.value.currentPlayer.substring(0, 8)]
        .chooseAction(state);
