    ...
    var newBoard = List.generate(
        ...
        (x) => List.generate(5, (y) {
              if (move.posX == x && move.posY == y)
                return Tile(move.player, state.board[x][y].charge + 1,
                    state.board[x][y].maxCharge, x, y);
...
class MinimaxAgent implements Agent {
  ...
  Future<MapEntry<int, Move>> minimax(
      int currentDepth, GameState state, bool isMaxTurn) async {
...
class WeightedRandomAgent implements Agent {
  ...
  Future<Move> chooseAction(GameState state) async {
    List<Move> actions = AiElements.actions(state);
    actions.shuffle();
    List<Move> weightedActions = <Move>[];
    for (Move move in actions) {
      ...
      if (state.board[move.posX][move.posY].owner ==
          move.player) //already owned: +1
        weightedActions.add(move);
    ...
    if (weightedActions
        .isEmpty) //in case no move gets a bonus and no move get random
...
class SimpleRuleAgent implements Agent {
  ...
  Future<Move> chooseAction(GameState state) async {
    ...
    for (Move move in actions) {
      if (state.board[move.posX][move.posY].maxCharge ==
              state.board[move.posX][move.posY].charge &&
    ...
    for (Move move in actions) {
      if (state.board[move.posX][move.posY].maxCharge ==
              state.board[move.posX][move.posY].charge + 1 &&
          !maxChargedEnemyTilesAround(move, state)) {
        return move;
      }
    }
  ...
  bool maxChargedEnemyTilesAround(Move move, GameState state) {
    if (move.posX < state.sizeX - 1 &&
        ...
        state.board[move.posX + 1][move.posY].maxCharge ==
            state.board[move.posX + 1][move.posY].charge) return true;
    if (move.posX > 0 &&
        state.board[move.posX - 1][move.posY].owner != state.currentPlayer &&
        state.board[move.posX - 1][move.posY].maxCharge ==
            state.board[move.posX - 1][move.posY].charge) return true;
    if (move.posY < state.sizeY - 1 &&
        ...
        state.board[move.posX][move.posY + 1].maxCharge ==
            state.board[move.posX][move.posY + 1].charge) return true;
    if (move.posY > 0 &&
        state.board[move.posX][move.posY - 1].owner != state.currentPlayer &&
        state.board[move.posX][move.posY - 1].maxCharge ==
            state.board[move.posX][move.posY - 1].charge) return true;
  ...
  bool enemyTilesAround(Move move, GameState state) {
    if (move.posX < state.sizeX - 1 &&
        state.board[move.posX + 1][move.posY].owner != state.currentPlayer &&
        state.board[move.posX + 1][move.posY].owner != "") return true;
    if (move.posX > 0 &&
        state.board[move.posX - 1][move.posY].owner != state.currentPlayer &&
        state.board[move.posX - 1][move.posY].owner != "") return true;
    if (move.posY < state.sizeY - 1 &&
        state.board[move.posX][move.posY + 1].owner != state.currentPlayer &&
        state.board[move.posX][move.posY + 1].owner != "") return true;
    if (move.posY > 0 &&
        state.board[move.posX][move.posY - 1].owner != state.currentPlayer &&
        state.board[move.posX][move.posY - 1].owner != "") return true;
    return false;
  ...
  Future<Move> chooseAction(GameState state) async {
    ...
    for (Move move in actions) {
      if (enemyTilesAround(move, state)) {
        ...
        if (state.board[move.posX][move.posY].maxCharge -
                state.board[move.posX][move.posY].charge >=
            enemyTile.maxCharge - enemyTile.charge) {
          return move;
        }
      }
    }
